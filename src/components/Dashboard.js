...
import TaskForm from './Tasks/TaskForm';
import TaskCard from './Tasks/TaskCard';
import { FaUser, FaBars, FaTasks, FaCheckCircle, FaRegClock } from 'react-icons/fa';
import emailjs from '@emailjs/browser';
...
const Dashboard = ({ onLogout }) => {
  ...
  // Initialize EmailJS
  useEffect(() => {
    emailjs.init("h7FW0ReYv7_70X8U5");
  }, []);
  ...
  // Fetch user and task data once auth state is ready
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        onValue(tasksRef, (snapshot) => {
          ...
          const progressValue = taskList.length ? Math.round((completed / taskList.length) * 100) : 0;
  ...
  const filteredTasks = () => {
    ...
    if (activeTab === 'remaining') return tasks.filter(t => !t.completed);
    return tasks;
  };
  ...
  if (!userData) return <div className="text-center mt-5">User data not found</div>;
  ...
  return (
    <div className="d-flex flex-column" style={{ width: '100%' }}>
      <div className="d-flex flex-grow-1 flex-column flex-lg-row" style={{ width: '100%' }}>
        <div className={`sidebar bg-light p-3 ${!showSidebar ? 'd-none' : 'd-lg-block'}`}
          style={{ borderRight: '1px solid #dee2e6' }}>
            {showSidebar && (
              <div className="d-flex align-items-center justify-content-between mb-4">
                <div className="d-flex align-items-center mb-4">
                  <Button variant="light" className="me-3 p-1 border d-lg-none" onClick={() => setShowSidebar(!showSidebar)}>
                    <FaBars />
                  </Button>
                  <h4 className="mb-0">Dashboard</h4>
              </div>
                <Button variant="primary" className='d-block d-lg-none' size="sm" onClick={() => { setEditingTask(null); setShowTaskForm(true); }}>
              + Add Task
            </Button>
                </div>
              )}
          <Card className="user-card mb-4 border-0 shadow-sm">
            <Card.Body className="text-center">
              <div className="bg-primary rounded-circle d-inline-flex align-items-center justify-content-center" style={{ width: 70, height: 70 }}>
                <FaUser style={{ fontSize: '1.8rem', color: 'white' }} />
              </div>
              <h5 className="mt-3 mb-1">{userData.name || 'User'}</h5>
              <small className="text-muted">{userData.email || auth.currentUser?.email}</small>
              <div className="d-grid gap-1 mt-3">
                <Button variant="primary" className='' onClick={() => { setEditingTask(null); setShowTaskForm(true); }}>
                  + Add New Task
                </Button>
              </div>
            </Card.Body>
        ...
        <div className="flex-grow-1 p-3" style={{ minHeight: 'calc(100vh - 56px)', backgroundColor: '#f8f9fa' }}>
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div className="d-flex align-items-center">
              {!showSidebar && (
                <>
                <Button variant="light" className="me-3 p-1 border" onClick={() => setShowSidebar(!showSidebar)}>
                  <FaBars />
                </Button>
                <h4 className="mb-0">Dashboard</h4>
                </>
            ...
            <Button variant="primary" className='d-none d-lg-block' size="sm" onClick={() => { setEditingTask(null); setShowTaskForm(true); }}>
              + Add Task
            </Button>
          </div>
          ...
          <Row className="mb-4 g-3">
            {[
              { label: 'Total Tasks', icon: <FaTasks />, count: totalTasks, tab: 'all' },
              { label: 'Completed', icon: <FaCheckCircle />, count: completedTasks, tab: 'completed' },
              { label: 'Remaining', icon: <FaRegClock />, count: remainingTasks, tab: 'remaining' }
            ].map(({ label, icon, count, tab }) => (
              <Col md={4} key={tab}>
                <Card className={`shadow-sm border-0 h-100 ${activeTab === tab ? 'border-primary' : ''}`} onClick={() => setActiveTab(tab)} style={{ cursor: 'pointer' }}>
                  <Card.Body className="d-flex align-items-center flex-grow-1 flex-wrap">
                    <div className="bg-light rounded p-3 me-3">{icon}</div>
                    <div>
          ...
          <Card className="shadow-sm border-0">
            <Card.Body>
              <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                  <h5 className="mb-0">{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Tasks</h5>
                  <small className="text-muted">{filteredTasks().length} tasks</small>
                </div>
                <Button variant="outline-primary" size="sm" onClick={() => setShowTaskForm(true)}>
                  + Add Task
                </Button>
              </div>
              ...
              {filteredTasks().length > 0 ? (
                  ...
                  {filteredTasks().map(task => (
                    <Col key={task.id} xs={12} sm={6} lg={4}>
                      <TaskCard
                        ...
                        setEditingTask={setEditingTask}
                        setShowTaskForm={setShowTaskForm}
                        refreshTasks={() => {}} // No need, listener updates automatically
                      />
                    </Col>
                  ))}
                </Row>
                ...
                <div className="text-center py-5">
                  <div className="bg-light rounded-circle d-inline-flex align-items-center justify-content-center p-4 mb-3">
                    {activeTab === 'completed' ? <FaCheckCircle size={32} className="text-muted" /> :
                      activeTab === 'remaining' ? <FaRegClock size={32} className="text-muted" /> :
                      <FaTasks size={32} className="text-muted" />}
                  </div>
                  <h5>No {activeTab} tasks</h5>
                  <p className="text-muted">Start by creating one</p>
                  <Button variant="primary" onClick={() => setShowTaskForm(true)}>Create Task</Button>
                </div>
              )}
            </Card.Body>
          </Card>
      ...
      <TaskForm
        show={showTaskForm}
        onHide={() => { setShowTaskForm(false); setEditingTask(null); }}
        editingTask={editingTask}
        setEditingTask={setEditingTask}
        refreshTasks={() => {}} // listener handles refresh
      />
    </div>
  );
};
